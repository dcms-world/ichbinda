...
function completePendingDeviceSwitch(personId,personName){localStorage.setItem('ibinda_person_id',personId);currentPersonId=personId;if(personName){setPersonName(personName);currentPersonName=personName}const url=new URL(window.location);url.searchParams.set('id',personId);window.history.replaceState({},'',url);renderPersonName();loadStatus(personId);loadWatchers(personId);loadDevices();setPendingDeviceSwitchMessage('Gerätewechsel abgeschlossen.',false);setTimeout(()=>{resetStatus()},2500)}
