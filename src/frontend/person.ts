...
<div id="inlineQrCard" style="display:none">
  <div class="inline-qr-code-wrap"><div id="inlineQrcode"></div></div>
</div>
...
function isInlineQrCardVisible(){const card=document.getElementById('inlineQrCard');return !!card&&card.style.display!=='none'}
...
function updatePrimaryActionButton(){const btn=document.getElementById('btnOkay');const status=document.getElementById('status');const inlineCard=document.getElementById('inlineQrCard');const inlineQrEl=document.getElementById('inlineQrcode');if(!btn)return;if(!hasActiveWatcherConnection&&cooldownInterval){clearInterval(cooldownInterval);cooldownInterval=null}btn.classList.remove('error');if(hasActiveWatcherConnection){btn.classList.remove('setup');btn.innerHTML='OK<span class="btn-sub">Alles gut</span>';btn.setAttribute('aria-label','Okay senden');btn.disabled=!!cooldownInterval;btn.style.display='';if(inlineCard)inlineCard.style.display='none';if(inlineQrEl)inlineQrEl.innerHTML='';if(status){status.style.display='';if(!cooldownInterval&&!(status.classList.contains('success')||status.classList.contains('error'))){status.className='status idle';status.textContent=getIdleStatusMessage()}}}else{btn.style.display='none';if(status){status.style.display='';status.className='status idle';status.textContent=getIdleStatusMessage()}if(inlineCard&&inlineCard.style.display==='none'){inlineCard.style.display='flex';if(inlineQrEl&&!inlineQrEl.innerHTML)renderQrCode(false)}}}
...
async function loadWatchers(personId){try{const res=await fetch(API_URL+'/person/'+encodeURIComponent(personId)+'/watchers');if(await isRealSessionLoss(res)){if(isAwaitingPendingDeviceSwitch())return;resetPersonApp('watchers-lost-ownership');return}if(!res.ok)return;const data=await res.json();const watchers=Array.isArray(data.watchers)?data.watchers:[];const disconnectEvents=Array.isArray(data.disconnect_events)?data.disconnect_events:[];const nameUpdates={};watchers.forEach((watcher)=>{if(watcher&&watcher.name&&watcher.id)nameUpdates[watcher.id]=watcher.name});disconnectEvents.forEach((event)=>{if(event&&event.watcher_name&&event.watcher_id)nameUpdates[event.watcher_id]=event.watcher_name});if(Object.keys(nameUpdates).length)cacheWatcherNames(nameUpdates);pendingDisconnectEvents=disconnectEvents;hasActiveWatcherConnection=watchers.length>0;updatePrimaryActionButton();updateDeviceActionSection({max_devices:currentPersonMaxDevices,device_count:currentPersonDeviceCount,device_action:currentPersonDeviceAction});const el=document.getElementById('watcherInfo');const warn=document.getElementById('noWatcherWarning');if(!watchers.length){if(el){el.innerHTML='<div class="settings-list"><div class="settings-item"><div class="device-meta">Noch nicht verbunden</div><button type="button" style="color:var(--system-blue);font-size:15px;font-weight:600;background:none;border:none;cursor:pointer" id="watcherQrEntryBtn">Einrichten</button></div></div>';const qrEntryBtn=document.getElementById('watcherQrEntryBtn');if(qrEntryBtn)qrEntryBtn.onclick=()=>openPairingQrModal()}if(warn)warn.classList.add('visible')}else{const count=watchers.length;const label=count===1?'1 Verbindung':count+' Verbindungen';const items=watchers.map((watcher)=>{const name=getWatcherDisplayName(watcher.id);const initials=name.charAt(0).toUpperCase();return '<div style="display:flex;align-items:center;padding:12px 0;border-top:0.5px solid var(--system-separator)">'+'<div style="width:36px;height:36px;border-radius:50%;background:var(--system-fill);display:flex;align-items:center;justify-content:center;margin-right:12px;font-size:15px;font-weight:600;color:var(--system-blue);flex-shrink:0">'+escapeHtml(initials)+'</div>'+'<div style="font-size:17px;font-weight:500;color:var(--system-label);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+escapeHtml(name)+'</div>'+'</div>'}).join('');if(el)el.innerHTML='<div class="settings-list"><div class="settings-item" style="cursor:pointer" id="watcherToggle"><div>'+label+'</div><div style="display:flex;align-items:center;gap:8px"><div style="color:var(--system-green);font-weight:600;font-size:14px">Verbunden</div><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--system-gray)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" id="watcherChevron" style="transition:transform 0.2s"><path d="M6 9l6 6 6-6"/></svg></div></div><div id="watcherIds" style="display:none;padding:0 16px 4px">'+items+'</div></div>';const watcherToggle=document.getElementById('watcherToggle');if(watcherToggle)watcherToggle.onclick=()=>{const idsEl=document.getElementById('watcherIds');const chevron=document.getElementById('watcherChevron');const isHidden=idsEl.style.display==='none';idsEl.style.display=isHidden?'block':'none';if(chevron)chevron.style.transform=isHidden?'rotate(180deg)':'rotate(0deg)'};if(warn)warn.classList.remove('visible')}if(disconnectEvents.length){if(!isPairingRequestModalOpen()&&!isDeviceSwitchModalOpen()&&!isPendingDeviceSwitchModalOpen())showDisconnectModal(disconnectEvents)}else if(isDisconnectModalOpen()){hideDisconnectModal()}}catch(e){console.error('loadWatchers failed',e)}}
