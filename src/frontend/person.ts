...
function completePendingDeviceSwitch(personId,personName){localStorage.setItem('ibinda_person_id',personId);currentPersonId=personId;currentDeviceId=getOrCreateDeviceId();if(personName){setPersonName(personName);currentPersonName=personName}const nextUrl=new URL(window.location.href);nextUrl.pathname='/person.html';nextUrl.searchParams.set('id',personId);window.location.replace(nextUrl.toString())}
